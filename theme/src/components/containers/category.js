...
const attributeTagUI = attrs => {
	console.log('attrs', attrs);
	let attributes = [];
	for (const prop in attrs) {
		Object.values(attrs[prop]).forEach(attrval => {
			attributes.push([prop, attrval]);
		});
		return attributes;
	}
};

const getattributeTag = productFilter => {
	let attrs = {};
	let attributeValue = '';
	let attributeName = '';
	let attributeValueFormatted = '';
	if (productFilter.attributes) {
		Object.keys(productFilter.attributes).forEach(attributeKey => {
			attributeName = attributeKey.replace('attributes.', '');
			attributeValue = productFilter.attributes[attributeKey];
...
const CategoryContainer = props => {
	...
	const attributeTag = getattributeTag(productFilter);
	console.log('attributeTag', attributeTag);
	const pageTitle =
