...
export const downBrochure = (productId, url) => {
	api.ajax.products.brochure
...
export const getProductFilterForSearch = locationSearch => {
	...
	return {
		categoryId: queryFilter.category,
		priceFrom: parseInt(queryFilter.price_from || 0),
		priceTo: parseInt(queryFilter.price_to || 0),
...
export const getParsedProductFilter = productFilter => {
	const filter = Object.assign(
		{},
		{
			...
			offset: 0,
			current_page: productFilter['current_page']
		},
...
export const enquiryForm = data => async (dispatch, getState) => {
	dispatch(requestProductForm());
	const response = await api.ajax.enquiry.create(data);
};

const requestProductForm = () => ({ type: t.PRODUCT_ENQUIRY_REQUEST });

export const contactusForm = data => async (dispatch, getState) => {
	dispatch(requestContactUsForm());
	const response = await api.ajax.contactus.create(data);
};

const requestContactUsForm = () => ({ type: t.CONTACTUS_FORM_REQUEST });
...
export const fetchProductsPage = page => (dispatch, getState) => {
	const { app } = getState();
	dispatch(setProductsFilter({ current_page: page }));
	dispatch(fetchProducts());
