...
const PRODUCT_FIELDS =
	'path,id,name,category_id,category_ids,category_name,sku,images,brochure,enabled,featured,discontinued,stock_status,stock_quantity,price,on_sale,regular_price,attributes,tags,position';
const CATEGORIES_FIELDS =
	'image,name,description,meta_description,meta_title,sort,parent_id,position,slug,id';

const BRANDS_FEILDS = 'id,name,image,position';
const BANNERS_FIELDS = 'id,name,image,position';
...
const getAllData = (currentPage, productFilter, cookie) => {
	return Promise.all([
		...
		getThemeSettings(),
		api.banners
			.list({ enabled: true, fields: BANNERS_FIELDS })
			.then(({ status, json }) => json),
		api.brands
			.list({ enabled: true, fields: BRANDS_FEILDS })
			.then(({ status, json }) => json)
	]).then(
		([
			checkoutFields,
...
const getState = (currentPage, settings, allData, location, productFilter) => {
	...
	if (products) {
		...
		productsCurrentPage = products.current_page;
		productsTotalPage = products.total_page;
