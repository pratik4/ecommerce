...
class ProductsService {
	...
	async getCategoryFromSearch(category) {
		const filter = {
			slug: category,
			enabled: true
		};
	...
	async getProducts(params = {}) {
		if (params.sort === 'search') {
			const categor = await this.getCategoryFromSearch(params.category_id);
			params.category_id = categor[0];
		}
		...
		const page = parse.getNumberIfPositive(params.current_page) || 1;
		const page_offset = limit * page - limit || 0;
		...
		itemsAggregation.push({ $skip: page_offset });
	...
	getMatchQuery(params, categories, useAttributes = true, usePrice = true) {
		...
		featured = parse.getBooleanIfValid(featured);
	...
	getValidDocumentForInsert(data) {
		...
		product.featured = parse.getBooleanIfValid(data.featured, true);
	...
	getBrochureWithUrls(item, domain) {
		if (item.brochure && item.brochure.length > 0) {
			let brochure = {
				filename: item.brochure
			};
			brochure.url = this.getBrochureUrl(domain, item.id, item.brochure || '');

			return brochure;
		} else {
			return item.brochure;
		}
	}
	...
	getBrochureUrl(domain, productId, brochureFileName) {
		return url.resolve(
			domain,
			`${settings.brochureUploadUrl}/${productId}/${brochureFileName}`
		);
	}
	...
	changeProperties(item, domain) {
		if (item) {
			...
			item.brochure = this.getBrochureWithUrls(item, domain);
