...
class SettingsService {
	constructor() {
		this.defaultSettings = {
			...
			product_fields:
				'path,id,name,category_id,category_name,sku,images,enabled,featured,discontinued,stock_status,stock_quantity,price,on_sale,regular_price,attributes,tags,position',
			products_limit: 30,
			weight_unit: 'kg',
			length_unit: 'cm',
	...
	getValidDocumentForUpdate(data) {
		...
		if (data.images !== undefined) {
			console.log('here in api server settings', data.images);
			settings.images = parse.getArrayIfValid(data.images);
