...
ajaxRouter.get('/brands', (req, res, next) => {
	api.brands.list(req.query).then({ status, json });
});

ajaxRouter.get('/brands/:id', (req, res, next) => {
	api.brands.retrieve(req.params.id).then(({ status, json }) => {
		res
			.status(status)
			.header('Cache-Control', DEFAULT_CACHE_CONTROL)
			.send(json);
...
ajaxRouter.get('/banners', (req, res, next) => {
	api.banners.list(req.query).then({ status, json });
});

ajaxRouter.get('/banners/:id', (req, res, next) => {
	api.banners.retrieve(req.params.id).then(({ status, json }) => {
		res
			.status(status)
			.header('Cache-Control', DEFAULT_CACHE_CONTROL)
			.send(json);
...
ajaxRouter.post('/enquiry', async (req, res, next) => {
	await api.enquiry.create(req.body).then(({ status, json }) => {
		res.status(status).send(json);
	});
});

ajaxRouter.post('/contactus', async (req, res, next) => {
	await api.contactus.create(req.body).then(({ status, json }) => {
		res.status(status).send(json);
	});
});
