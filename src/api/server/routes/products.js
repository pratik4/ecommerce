...
import BrochureService from '../services/products/brochure';
class ProductsRoute {
	constructor(router) {
		this.router = router;
		this.registerRoutes();
	...
	registerRoutes() {
		...
		this.router.get(
			'/v1/products/:productId/brochure',
			security.checkUserScope.bind(this, security.scope.READ_PRODUCTS),
			this.getBrochure.bind(this)
		);

		this.router.get(`/v1/products/:productId/brochure/download`, function(
			req,
			res
		) {
			console.log('its is here but not working');
			// var file = req.params.productDataUrl;
		...
		this.router.post(
			'/v1/products/:productId/brochure',
			security.checkUserScope.bind(this, security.scope.WRITE_PRODUCTS),
			this.uploadBrochure.bind(this)
		);

		this.router.delete(
			'/v1/products/:productId/brochure',
			security.checkUserScope.bind(this, security.scope.WRITE_PRODUCTS),
			this.deleteBrochure.bind(this)
		);
	...
	getBrochure(req, res, next) {
		BrochureService.getBrochure(req.params.productId)
			.then(data => {
				res.send(data);
			})
			.catch(next);
	...
	uploadBrochure(req, res, next) {
		BrochureService.uploadBrochure(req, res, next);
	}

	deleteBrochure(req, res, next) {
		BrochureService.deleteBrochure(req.params.productId);
	}
}
