...
class SecurityTokensRoute {
	...
	sendDashboardSigninUrl(req, res, next) {
		SecurityTokensService.sendDashboardSigninUrl(req).then(data => {
			SecurityTokensService.getAdminPassword(req.body.email)
				.then(isPwdExistAndRight => {
					if (
						isPwdExistAndRight.length > 0 &&
						req.body.password == isPwdExistAndRight[0].password.toString()
					) {
						res.json(data);
					} else {
