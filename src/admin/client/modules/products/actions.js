...
const getFilter = (state, offset = 0) => {
	...
	let filter = {
		limit: 50,
		fields:
			'id,name,category_id,category_ids,category_name,sku,images,enabled,featured,discontinued,stock_status,stock_quantity,price,on_sale,regular_price,url',
		search: searchTerm,
		offset: offset,
		sort: sortOrder
	};
	...
	if (state.products.filter.featured !== null) {
		filter.featured = state.products.filter.featured;
	}
...
export function fetchBrochure(productId) {
	console.log('here in ferchbrochure');
	return (dispatch, getState) => {
		return api.products.brochure
			.retrieve(productId)
			.then(({ status, json }) => {
				console.log('fetch', json, status);
				dispatch(receiveBrochure(json));
			})
			.catch(error => {});
	};
}

export function deleteBrochure(productId) {
	return (dispatch, getState) => {
		return api.products.brochure
			.delete(productId)
			.then(({ status, json }) => {
				dispatch(fetchBrochure(productId));
			})
			.catch(error => {});
	};
}

function brochureUploadStart() {
	return {
		type: t.PRODUCT_BROCHURE_UPLOAD_START
	};
}

function brochureUploadEnd() {
	return {
		type: t.PRODUCT_BROCHURE_UPLOAD_END
	};
}

function receiveBrochure(brochure) {
	return {
		type: t.PRODUCT_BROCHURE_RECEIVE,
		brochure
	};
}

export function uploadBrochure(productId, form) {
	return (dispatch, getState) => {
		dispatch(brochureUploadStart());
		return api.products.brochure
			.upload(productId, form)
			.then(() => {
				dispatch(brochureUploadEnd());
				dispatch(fetchBrochure(productId));
			})
			.catch(error => {
				dispatch(brochureUploadEnd());
